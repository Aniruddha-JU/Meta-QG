...
from preprocessorQA import Corpus, Reprer
from learnerQA import LearnerQA
...
def train_NOmeta(args):
    ...
    corpus_en_train = Corpus('bert-base-multilingual-cased', args.max_seq_len, args.doc_stride, args.max_query_length, logger, language='en', mode='train', is_training = True, support_size=-1, base_features=None, mask_rate=args.mask_rate,
                             compute_repr=False, shuffle=True)

    corpus_en_valid = Corpus('bert-base-multilingual-cased', args.max_seq_len, args.doc_stride, args.max_query_length, logger, language='en', mode='dev', is_training = False, support_size=-1, base_features=None, mask_rate=-1.0, compute_repr=False, shuffle=False)
    # build the model
    freeze_layer = 0
    learner = LearnerQA(args.bert_model, args.freeze_layer, args.hidden_size, logger, args.lr_meta, args.lr_inner,
    ...
    for step in range(args.max_meta_steps):
        ...
        loss = learner.forward_NOmeta(batch_data)
        ...
        if step % args.eval_every_meta_steps == 0:
            ...
            F1_valid = learner.evaluate_NOmeta(corpus_en_valid, args.result_dir, logger, is_training=False, lang='en',mode='dev')
...
def train_meta(args):
    ...
    ## prepare dataset
    corpus_en_train = Corpus('bert-base-multilingual-cased', args.max_seq_len, args.doc_stride, args.max_query_length, logger, language='en', mode='train', is_training = False, support_size=args.support_size, base_features=None, mask_rate=args.mask_rate,
    ...
    for step in range(args.max_meta_steps):
        ...
        batch_query, batch_support, ex, fe = corpus_en_train.get_batch_meta(batch_size=args.inner_size)#(batch_size=32)
        loss = learner.forward_meta(batch_query, batch_support, progress=progress, inner_steps=args.inner_steps,          lambda_max_loss=args.lambda_max_loss, lambda_mask_loss=args.lambda_mask_loss)

        if step % 1 ==0:
            logger.info('Step: {}/{}, loss = {:.6f}, time = {:.2f}s.'.format(step, args.max_meta_steps, loss, time.time()-t))

        if step % args.eval_every_meta_steps==0 and step > 3000 :
...
#         assert False, 'Already evaluated.'
    ...
    corpus_en_train = Corpus('bert-base-multilingual-cased', args.max_seq_len, args.doc_stride, args.max_query_length, logger, language='en', mode='train', is_training = True, support_size=-1, base_features=None, mask_rate=-1.0, compute_repr=True, shuffle=False, reprer=reprer)

    learner = LearnerQA(args.bert_model, args.freeze_layer, args.hidden_size, logger, args.lr_meta, args.lr_inner,
                      args.warmup_prop_meta, args.warmup_prop_inner, args.max_meta_steps,
                      model_dir=args.model_dir, gpu_no=args.gpu_device).to(device)

    languages = args.test_langs
    F1s = {lang: [] for lang in languages}
    for lang in languages:
        corpus_test = Corpus('bert-base-multilingual-cased', args.max_seq_len, args.doc_stride, args.max_query_length, logger, language=lang, mode='dev', is_training = False, support_size=args.support_size, base_features=corpus_en_train.original_features, mask_rate=-1.0, compute_repr=True, shuffle=False, reprer=reprer)
...
#         assert False, 'Already evaluated.'
    ...
    learner_pretrained = LearnerQA(args.bert_model, args.freeze_layer, args.hidden_size, logger, lr_meta=0, lr_inner=0,
                                 warmup_prop_meta=-1, warmup_prop_inner=-1, max_meta_steps=-1,
                                 model_dir=args.model_dir, gpu_no=args.gpu_device).to(device)
    ...
    for lang in languages:
        corpus_train = Corpus('bert-base-multilingual-cased', args.max_seq_len, args.doc_stride, args.max_query_length, logger, language=lang, mode='train',  is_training = False, support_size=-1, base_features=None, mask_rate=-1.0,
                              compute_repr=False, shuffle=True, k_shot_prop=args.k_shot) # add k_shot_prop
        corpus_test = Corpus('bert-base-multilingual-cased', args.max_seq_len, args.doc_stride, args.max_query_length, logger, language=lang, mode='dev', is_training = False, support_size=-1, base_features=None, mask_rate=-1.0,
                             compute_repr=False, shuffle=False)
        ...
        for ft_step in range(args.max_ft_steps):
            data_batches, examples, features = corpus_train.get_batches(batch_size=8, device="cuda", shuffle=True)

            for batch_data in data_batches:
                #print("Haha length of batch size: ",len(batch_data))
                #print(batch_data)
                #print('\n\n\n')
            #if ft_step%10==0 or ft_step==0:
                loss = learner.inner_update(batch_data, lr_curr=args.lr_finetune, inner_steps=1)
                #F1_test = learner.evaluate_NOmeta(corpus_test, args.result_dir, logger, lang=lang, mode='dev')
            if ft_step==24  :
                ...
                F1_test = learner.evaluate_NOmeta(corpus_test, args.result_dir, logger, lang=lang, mode='dev')
                F1s[lang].append(F1_test)
                #logger.info("===> Test F11: {}".format(F1_test_1))
                logger.info("===> Test F12: {}".format(F1_test))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--inner_size', type=int, help='[number of tasks] for one meta-update', default=16)
    ...
    parser.add_argument('--max_meta_steps', type=int, help='maximal steps token for meta training.', default=6001)
    parser.add_argument('--eval_every_meta_steps', type=int, default=200)
    ...
    # permanent params
    parser.add_argument('--freeze_layer', type=int, help='the layer of mBERT to be frozen', default=0)
    parser.add_argument('--max_seq_len', type=int, default=512)
    parser.add_argument("--hidden_size", default=768, type=int, help="Hidden size for discriminator")
    parser.add_argument("--max_query_length",type=int, default=48, help="Max query length")
...
#             assert False, 'Existing result directory!'
    ...
    if args.zero_shot:
        ...
        if args.no_meta_learning:
            ...
            print("metalearningggggggggggggggg")
            train_meta(args)
